...
window.addEventListener(
  ...
  function () {
    ...
    (function () {
      var i;
      for (i = 0; i < 8; ++i) {
        create_texture("./image/matcap" + i + ".jpg", i);
      }
    ...
    function objLoad() {
      ...
      xhr.open("GET", "./bunny.obj");
      xhr.onload = function () {
        var res = xhr.response;
        // objsonのインスタンスを生成
        var objson = new OBJSON();
        // 取得したモデルデータをパース
        bunny = objson.objsonConvert(res);
        // 初期化関数を呼ぶ
        init();
      };
      xhr.send();
    ...
    function init() {
      ...
      // モデルデータから VBO などを生成
      var position = create_vbo(bunny.data[0].position);
      var normal = create_vbo(bunny.data[0].normal);
      ...
      function render() {
        ...
        gl.drawArrays(gl.TRIANGLES, 0, bunny.data[0].position.length / 3);
