(function () {
  ...
  window.addEventListener(
    ...
    function () {
      ...
      // texture setting
      create_texture("map.jpg", init);

      function init(texture) {
        // default shader(vs and fs)
        var vs = create_shader("vs");
        var fs = create_shader("fs");
        var prg = create_program(vs, fs);
        var uniLocation = [];
        uniLocation[0] = gl.getUniformLocation(prg, "mvpMatrix");
        uniLocation[1] = gl.getUniformLocation(prg, "texture2dSampler");

        // centroid shader(vsc and fsc)
        var vsc = create_shader("vsc");
        var fsc = create_shader("fsc");
        var cPrg = create_program(vsc, fsc);
        var cUniLocation = [];
        cUniLocation[0] = gl.getUniformLocation(cPrg, "mvpMatrix");
        cUniLocation[1] = gl.getUniformLocation(cPrg, "texture2dSampler");
        ...
        function render() {
          ...
          mat.perspective(60, 0.5, 0.1, camera.cameraDistance * 5.0, pMatrix);
          ...
          var radian = (nowTime * 0.05) % (Math.PI * 2.0);
          ...
          gl.uniformMatrix4fv(cUniLocation[0], false, mvpMatrix);
          gl.uniform1i(cUniLocation[1], 0);
          gl.drawElements(
            gl.TRIANGLES,
            planeIndex.length,
            gl.UNSIGNED_SHORT,
